...
#+begin_src emacs-lisp
  (defun claude-generate-commit-message ()
    ...
    (let* ((git-diff (shell-command-to-string "git diff --cached"))
           ...
           (prompt (format "请根据以下 git diff 和状态信息生成一个简洁的提交信息（只返回提交信息，不要其他解释）：\n\nGit Status:\n%s\n\nGit Diff:\n%s"
                          git-status git-diff)))
        ...
        (let ((temp-file (make-temp-file "claude-commit-" nil ".txt")))
          ...
          (let* ((proxy-env (getenv "EMACS_HTTP_PROXY"))
            ...
            (if (and commit-msg (not (string-empty-p commit-msg)))
                (progn
                  (message "生成的提交信息: %s" commit-msg)
                  (insert commit-msg))
              (message "Claude 生成提交信息失败")))))))
  ...
  (use-package magit
    ...
    (defun magit-commit-with-claude-suggestion ()
      "提交时提示使用 Claude 生成提交信息"
      (interactive)
      (magit-commit-create)
      (when (y-or-n-p "是否使用 Claude 生成提交信息？")
...
#+begin_src emacs-lisp
  (use-package yasnippet-snippets
    :hook ((ls-bridge-mode . yas-minor-mode))
    :config
    (yas-reload-all))
#+end_src
** gradle
  	...
  	(defun find-lombok-jar ()
  		...
  		(let* ((m2-repo (expand-file-name "~/.m2/repository/org/projectlombok/lombok/"))
  					 ...
  					 (lombok-jar (when latest-version
  												 (expand-file-name 
  													(format "%s/lombok-%s.jar" latest-version latest-version)
