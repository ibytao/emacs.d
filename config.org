...
** proxy
#+begin_src emacs-lisp
  (let ((proxy (getenv "EMACS_HTTP_PROXY")))
...
#+begin_src emacs-lisp
  ...
  (defun my-switch-to-buffer ()
    "Switch to a buffer, filtering out vterm-mode buffers."
    (interactive)
    (let ((completion-predicate
           (lambda (buffer)
             (and (not (eq (buffer-local-value 'major-mode buffer) 'vterm-mode))
                  (string-match-p "\\`[^*]" (buffer-name buffer))))))
      (call-interactively 'persp-ivy-switch-buffer completion-predicate)))
