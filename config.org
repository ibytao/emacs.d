...
#+begin_src emacs-lisp
  (defun claude-generate-commit-message ()
    ...
    (let* ((git-diff (shell-command-to-string "git diff --cached"))
           ...
           (prompt (format "Generate a concise git commit message based on the following git diff and status. Return only the commit message, no other explanation:\n\nGit Status:\n%s\n\nGit Diff:\n%s"
                          git-status git-diff)))
      (if (string-empty-p git-diff)
        ...
        (let ((temp-file (make-temp-file "claude-commit-" nil ".txt")))
          ...
          (let* ((proxy-env (getenv "EMACS_HTTP_PROXY"))
                 ...
                 (commit-msg (string-trim (shell-command-to-string command))))
